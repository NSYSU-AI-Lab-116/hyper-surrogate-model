...
def calculate_metrics(predictions_path: str):
    ...
    try:
        ...
        logger.error(
            f"Error: File not found at '{predictions_path}'. Please check the file path."
        )
        return
    ...
    print(
        "   (A key metric for surrogate models. Closer to 1.0 means better ranking ability.)"
    )
    print("-" * 60)
    ...
    plt.scatter(
        ...
        predicted_values,
        alpha=0.3,
        label=f"Spearman's ρ = {spearman_corr:.4f}",
    )
