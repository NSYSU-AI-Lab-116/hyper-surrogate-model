...
def acc_trainer() -> None:
    ...
    progress_bar = tqdm(range(max_train_steps), desc="Epochs", leave=True, position=1)
    gpu_util = tqdm(
        total=0,
        desc="Device util",
        leave=True,
    ...
    for epoch in range(EPOCHS):
        ...
        total_loss = 0
        batch_bar = tqdm(
            range(len(train_dataloader)), desc="Batches", leave=False, position=2
        )
        ...
        progress_bar.set_postfix(
            {"epoch": f"{epoch + 1}/{EPOCHS}", "loss": f"{epoch_avg_loss:.4f}"}
        )
        scheduler.step()
